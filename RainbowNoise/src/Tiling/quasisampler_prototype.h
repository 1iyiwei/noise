...
public:
  ...
  Point2D(){color=0;};
  Point2D(const double x, const double y) { this->x=x; this->y=y; color=0;}
  Point2D(const double vect[2]) { x=vect[0]; y=vect[1]; color=0;}
...
protected:
  ...
  static double correction_amount; // 1.0 by default, 0.0 for no correction
  ...
  static Point2D calcDisplacementVector(unsigned importance, unsigned f_code, int dir)
  {
    ...
    return  
      vvect[dir]        * lut[i_v][i_s][0] * correction_amount + // u component
      vvect[(dir+5)%20] * lut[i_v][i_s][1] * correction_amount ; // v component
  }  
...
public:
  ...
  class TileNode
  {
    ...
    Point2D p4; // together with the 3 points above form a rhomb
    Point2D p_rhomb; // center of the original containing rhomb
    ...
    unsigned color; // for colorization
  ...
  public:
    ...
    Point2D getPRhomb() const { return p_rhomb; }
    ...
    bool isCollectibleRhombType() const {
      return ( (tileType == TileTypeC) || (tileType == TileTypeE) ); }
    bool isRhombType() const {
      return ( (tileType == TileTypeC) || (tileType == TileTypeD) || (tileType == TileTypeE) || (tileType == TileTypeF) ); }
    // http://local.wasp.uwa.edu.au/~pbourke/geometry/polyarea/
    double det() const {return (p1.x*p2.y - p2.x*p1.y + p2.x*p3.y - p3.x*p2.y + p3.x*p1.y - p1.x*p3.y);}
    double rhombArea() const {return det();}
    unsigned getLevel() { return level; }
    bool isTerminal() const { return terminal; }
    TileNode* getParent() { return parent; }
    unsigned numChildren(void) const {return children.size();}
    TileNode* getChild(unsigned i) {  return children[i]; }
    unsigned numNeighbors(void) const {return neighbors.size();}
    TileNode* getNeighbor(unsigned i)
    {  
        if(i < neighbors.size()) return neighbors[i];
        else return 0;
    }
    unsigned getColor(void) const { return color;}
    void setColor(const unsigned input_color) {color = input_color; p_rhomb.color = input_color;}

    // return 0 if anyone is not a rhomb
    // else return the number of vertex-pairs close enough to each other
    int isRhombNeighbor(const TileNode * query, const double delta) const
    {
        if(! query) return 0;

        if((!(query->isRhombType())) || (!(this->isRhombType()))) return 0;

        vector<Point2D> query_points, my_points;

        query_points.push_back(query->getP1());
        query_points.push_back(query->getP2());
        query_points.push_back(query->getP3());
        query_points.push_back(query->getP4());

        my_points.push_back(this->getP1());
        my_points.push_back(this->getP2());
        my_points.push_back(this->getP3());
        my_points.push_back(this->getP4());

        int count = 0;

        Point2D diff;
        for(unsigned int i = 0; i < query_points.size(); i++)
            for(unsigned int j = 0; j < my_points.size(); j++)
            {
                diff = query_points[i] - my_points[j];

                if(diff.d2() < delta) count++;
            }
 
        return count;
    }

    void addRhombNeighbor(TileNode * query, const double delta)
    {
        const int count = isRhombNeighbor(query, delta);

#if 0
        // incorrect; should use some sort of intersection test
        if(count <= 0)
        {
            // do nothing
        }
...
#endif
        {
            if((count == 2) && query->isCollectibleRhombType() && this->isCollectibleRhombType() && !query->numChildren() && !this->numChildren())
            {
                int not_already_neighbor = 1;

                for(unsigned int i = 0; i < neighbors.size(); i++)
                {
                    if(neighbors[i] == query) not_already_neighbor = 0;
                }

                if(not_already_neighbor) neighbors.push_back(query);
            }

            for(unsigned int i = 0; i < children.size(); i++)
            {
                children[i]->addRhombNeighbor(query, delta);
            }
        }
    }
  ...
  Quasisampler(double width=0.0, double height=0.0, double correction_amount_input = 1.0) 
  { this->width=width; this->height=height; root=NULL; correction_amount = correction_amount_input;}
...
public:
  ...
  int buildNeighborGraph(const double input_delta)
  {  
      // figure out the minimum tile scale
      double min_scale = 2 * MAX(width, height);
      {
          TileLeafIterator it(root);
    
          do 
          {
              if(it->getScale() < min_scale) min_scale = it->getScale();
          } 
          while(it.next());
      }

      // build neighbor graph
      const double delta = pow(min_scale*input_delta, 2.0);
      {
          TileLeafIterator it(root);
    
          do 
          {
              root->addRhombNeighbor(*it, delta);
          } 
          while(it.next());
      }
    ...
    bool filterBounds = true )
  {
    ...
    do {
      ...
      {   
        ...
        if ( all_points || (importance >= calcFCodeValue( it->getFCode() , 2*it->getLevel() )) )
        {
    ...
    bool filterBounds = true )
  {
    ...
    do {
      pt = it->getP1();  
      if ( (all_rhombs && it->isRhombType()) || (it->isCollectibleRhombType()) )
      {   
        importance = getImportanceAt_bounded( pt );
        ...
        // if ( importance >= calcFCodeValue( it->getFCode() , 2*it->getLevel() ) )
        {
          ...
          pt_displaced = it->getPRhomb(); // (it->getP1() + it->getP2())*0.5;
...
public:
  ...
  std::vector<Point2D> getSamplingPoints(bool all_points = 0)
  {
    std::vector<Point2D> pointlist;
    
    // buildAdaptiveSubdivision(); 
    collectPoints(pointlist, all_points);
    return pointlist;
  }

 std::vector<Point2D> getRhombPoints(bool all_rhombs = 0)
  {
    std::vector<Point2D> pointlist;
    
    // buildAdaptiveSubdivision(); 
    collectRhombPoints(pointlist, all_rhombs);
    return pointlist;
  }

  std::vector<TileNode *> getLeafNodes(bool collectible_rhomb_only = 1)
  {
      std::vector<TileNode *> leafnodes;

      TileLeafIterator it(root);
      do 
      {
          if((!collectible_rhomb_only && it->isRhombType()) || it->isCollectibleRhombType())
          {
  ...
  // see http://facstaff.unca.edu/mcmcclur/mathematicaGraphics/PenroseColoring/ for flip_probability, i.e. usually 90%
  // return the total number of iterations actually run
  int colorize(const unsigned num_colors, const double flip_probability, const int num_iterations);
...
#include "quasisampler_colorize.h"
...
double Quasisampler::correction_amount = 1.0;
