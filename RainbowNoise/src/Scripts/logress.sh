...
./src/Main/Release/WhiteNoiseSamples ${dimension} ${num_class_plus_1} ${num_samples} ${num_points} ${domain_size} > ${output_dir}/${base_name}_init_points.txt

./src/Main/Release/RandomPickPointsSites ${output_dir}/${base_name}_init_points.txt ${dimension} ${num_class} ${num_samples} > ${output_dir}/${base_name}_points.txt

foreach voronoi_scheme (lloyd)
foreach centroid_scheme (snap)
foreach class_weight_scheme (fair rainbow single all)

set output_base_name = ${base_name}_${voronoi_scheme}_${class_weight_scheme};

if(${class_combo} > 0) then
./src/Main/Release/LloydRelaxationPoints ${output_dir}/${base_name}_points.txt ${dimension} ${domain_size} ${voronoi_scheme} ${centroid_scheme} ${class_weight_scheme} ${min_movement} ${max_num_iterations} ${class_combo} ${output_dir}/${output_base_name}_points.txt > ${output_dir}/${output_base_name}_samples.txt
else
./src/Main/Release/LloydRelaxation ${output_dir}/${base_name}_points.txt ${dimension} ${domain_size} ${voronoi_scheme} ${centroid_scheme} ${class_weight_scheme} ${min_movement} ${max_num_iterations} > ${output_dir}/${output_base_name}_samples.txt
endif
