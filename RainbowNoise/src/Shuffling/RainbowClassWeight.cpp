...
RainbowClassWeight::RainbowClassWeight(const vector<int> & num_samples_per_class) : SampleCountClassWeight(num_samples_per_class)
{
    // nothing to do
}
...
float RainbowClassWeight::Get(const int class_id0, const int class_id1) const
{
    ...
    return weight*((count == 1) || (count == 2));
}

float RainbowClassWeight::Get(const vector<int> & source_classes) const
{
    ...
    // return weight*((count == 1) || (count == _num_samples.size()));
    // 08/28/2009: dart throwing consider only pairs of classes (same/diff id)
    return weight*((count == 1) || (count == 2));
}
