MACRO(COPY_FILE_IF_CHANGED in_dir in_file out_file target)
    IF(${in_file} IS_NEWER_THAN ${out_file})
        #MESSAGE("${target} copy: ${in_dir}/${in_file} -> ${out_file}")
    	ADD_CUSTOM_COMMAND (
            TARGET     ${target}
            POST_BUILD
    	    COMMAND    ${CMAKE_COMMAND} -E copy ${in_dir}/${in_file} ${out_file}
    	)
	ENDIF(${in_file} IS_NEWER_THAN ${out_file})
ENDMACRO(COPY_FILE_IF_CHANGED)

MACRO(COPY_FILE_INTO_DIRECTORY_IF_CHANGED in_dir in_file out_dir target)
	GET_FILENAME_COMPONENT(file_name ${in_file} NAME)
	COPY_FILE_IF_CHANGED(${in_dir} ${in_file} ${out_dir}/${file_name}
${target})	
...
MACRO(COPY_FILES_INTO_DIRECTORY_IF_CHANGED in_dir in_file_list out_dir target)
    FOREACH(in_file ${in_file_list})
        #MESSAGE("${target} copy: ${in_dir}/${in_file} -> ${out_dir}")
	COPY_FILE_INTO_DIRECTORY_IF_CHANGED(${in_dir} ${in_file} ${out_dir} ${target})
    ENDFOREACH(in_file)
...
MACRO(COPY_DIRECTORY_IF_CHANGED in_dir out_dir target)
	...
	FOREACH(in_file ${in_file_list})
	    if(NOT ${in_file} MATCHES ".svn")
    		...
    		COPY_FILE_IF_CHANGED(${in_dir} ${in_file} ${out_file} ${target})
