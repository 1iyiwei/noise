...
int Main(int argc, char **argv)
{
    if(argc < 3)
    {
        cerr << "Usage: " << argv[0] << " spectrum-file1 [spectrum-file2-n] output-spectrum-name" << endl;
        return 1;
    }

    const char * output_file_name = argv[argc-1];
    
    vector<const char *> input_file_names;
    {
    ...
    if(! FrameBuffer::ReadPFM(input_file_names[0], fourier_magnitude_image))
    {
    ...
    for(unsigned int which_input = 0; which_input < input_file_names.size(); which_input++)
    {
        ...
        // read in
        if(! FrameBuffer::ReadPFM(input_file_names[which_input], fourier_magnitude_image))
        {
    ...
    if(ppm_or_pfm == 0)
    {
        ...
        // remove DC component
        {
            vector<int> output_index = spectrum_size;

            for(unsigned int i = 0; i < output_index.size(); i++)
            {
                output_index[i] = spectrum_size[i]/2;
            }
            ...
            if(maximum > 0)
            {
                for(unsigned int i = 0; i < output.size(); i++)
                    for(unsigned int j = 0; j < output[i].size(); j++)
                    {
                        ...
                        output[i][j].r = output[i][j].g = output[i][j].b = static_cast<int>(floor(pixel/maximum * 255));
                    }
            }
        }
