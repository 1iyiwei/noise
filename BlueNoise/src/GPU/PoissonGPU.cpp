/*
  ...
  the main driver for GPU Poisson disk sampling generation
...
void Reshape(int width, int height)
{
    ...
    CgPoisson * p_old_poisson = house_keeper.p_poisson;

    {
        for(unsigned int i = 0; i < house_keeper.initial_noise_keys.size(); i++)
        {
            house_keeper.initial_noise_keys[i] = CgPoisson::Hash(house_keeper.initial_noise_keys[(i+house_keeper.initial_noise_keys.size()-1)%house_keeper.initial_noise_keys.size()]);
        }
    }
    
    house_keeper.p_poisson = new CgPoisson(house_keeper.texture_size, house_keeper.cell_size, house_keeper.clique_size, house_keeper.neighbor_offsets, house_keeper.boundary_condition, house_keeper.neighbor_offset_storage, house_keeper.initial_noise_keys, house_keeper.r_value_texture_size);
...
CgPoisson::TextureSize ComputeLargestTextureSize(const CgPoisson::TextureSize & texture_size, const float init_cell_size, const float min_cell_size)
{
    // largest possible texture size
    CgPoisson::TextureSize largest_texture_size = texture_size;
...
int GetDistanceField(const int dimension, const float min_distance, const string & importance_field_name, const vector<float> & range_min, const vector<float> & range_max, Array<float> & distance_field_data, const CgPoisson::TextureSize & texture_size, const float init_cell_size, const float min_cell_size)
{
    if(importance_field_name == "ramp")
    {
        const CgPoisson::TextureSize largest_texture_size = ComputeLargestTextureSize(texture_size, init_cell_size, min_cell_size);
...
int Main(int argc, char **argv)
{
    ...
    const CgPoisson::NeighborOffsetStorage neighbor_offset_storage = static_cast<CgPoisson::NeighborOffsetStorage>(atoi(argv[++argCtr]));
    const CgPoisson::NoiseOption noise_option = static_cast<CgPoisson::NoiseOption>(atoi(argv[++argCtr]));
    ...
    if(! CgPoisson::ConvertTextureSize(tree_texture_size, texture_size))
    {
    ...
    glutCreateWindow("Blue Noise - Poisson Disk Sampling");
...

    {
        ...
        const CgPoisson::TextureSize init_texture_size(texture_size);
        ...
        house_keeper.init_texture = vector<GLuint>(CgPoisson::RequiredDrawBuffers(dimension));
        ...
        house_keeper.init_texture = CgPoisson::GenerateInitTexture(init_texture_size, tree.CellSize(tree.NumLevels()-1), clique_size, samples, house_keeper.init_texture);
        ...
        if((noise_option == CgPoisson::MD5_NOISE) && glewIsSupported("GL_EXT_gpu_shader4"))
        {
            house_keeper.initial_noise_keys = vector<int>(CgPoisson::RequiredDrawBuffers(dimension));
            ...
            {
                house_keeper.initial_noise_keys[i] = CgPoisson::Hash(house_keeper.initial_noise_keys[i-1]);
            }
        }
        else
        {
        ...
        if(importance_field_name == "")
        {
            ...
            house_keeper.r_value_texture_size = CgPoisson::TextureSize(zero, zero);
        }
        else
        {
            ...
            CgPoisson::TextureSize importance_field_texture_size;
            if(!CgPoisson::ConvertTextureSize(importance_field_size, importance_field_texture_size))
            {
                throw Exception("CgPoisson::ConvertTextureSize(importance_field_size, importance_field_texture_size)");
            }
            const CgPoisson::TextureSize r_value_texture_size = is_uniform ? CgPoisson::TextureSize(zero, zero) : importance_field_texture_size;

            house_keeper.r_value_texture = CgPoisson::GenerateRValueTexture(importance_field_texture_size, importance_field_data, house_keeper.r_value_texture);
    ...
    if(house_keeper.initial_noise_keys.size() <= 0)
    {
        ...
        // largest possible texture size
#if 1
        const CgPoisson::TextureSize largest_texture_size = ComputeLargestTextureSize(house_keeper.texture_size, house_keeper.cell_size, house_keeper.min_cell_size);
#else   
        CgPoisson::TextureSize largest_texture_size = house_keeper.texture_size;
        ...
        for(int i = 0; i < k_number; i++)
        {
            ...
            for(int j = 0; j < CgPoisson::RequiredDrawBuffers(dimension); j++)
            {
                GLuint texture_id = -1;
    ...
    if(house_keeper.initial_noise_keys.size() <= 0)
    {
        house_keeper.white_noise_texture = vector<GLuint>(CgPoisson::RequiredDrawBuffers(dimension));
