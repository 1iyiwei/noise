...
./src/Main/Release/DartThrowing ${domain_option}l${p_value} ${dimension} ${r_value} ${k_value} ${domain_size} ${boundary} ${sample_grid_resolution_per_unit_domain} > ${output_dir}/${base_name}_samples.txt;
...
./src/Main/Release/DartThrowing ${domain_option}l${p_value} ${dimension} ${r_value} ${k_value} ${domain_size} ${boundary} ${sample_grid_resolution_per_unit_domain} > ${huge_dir}/${base_name}_${i}.txt;
...
if(${xform} == "l2") then
./src/Main/Release/UniformDiffXform ${output_dir}/all_sample_files_list.txt ${dimension} ${sub_domain_min} ${sub_domain_max} ${diff_domain_max} ${kernel_option} ${kernel_size} ${keep_dc} ${output_resolution} ${output_dir}/${output_base_name}_average${num_runs}.f2m;
else
./src/Main/Release/UniformLpDiffXform ${p_value} ${local_diff_range} ${mean_dist} ${diff_norm} ${diff_weight} ${output_dir}/all_sample_files_list.txt ${dimension} ${sub_domain_min} ${sub_domain_max} ${diff_domain_max} ${kernel_option} ${kernel_size} ${keep_dc} ${output_resolution} ${output_dir}/${output_base_name}_average${num_runs}.f2m;
endif

rm ${output_dir}/all_sample_files_list.txt;

./src/Main/Release/F2M2PFM ${output_dir}/${output_base_name}_average${num_runs}.f2m ${output_dir}/${output_base_name}_average${num_runs}.pfm;

./src/Main/Release/PFM2PPM ${output_dir}/${output_base_name}_average${num_runs}.pfm ${output_dir}/${output_base_name}_average${num_runs}.ppm 1 1 0

echo ${output_dir}/${output_base_name}_average${num_runs}.ppm
