...
double Math::Factorial(const int low, const int high)
{
    double answer = 1;
...
double Math::Choose(const int n, const int k)
{
    // return Factorial(n)/(Factorial(k)*Factorial(n-k));
    
    const int big = max(k, n-k);
    const int small = min(k, n-k);

    return Factorial(big+1, n)/Factorial(small);
}
